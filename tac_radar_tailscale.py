...
def run_hackrf_worker(args):
    """Synchronous subprocess reader with aggressive process-killing cleanup routing."""
    ...
    while True:
        ...
        try:
            ...
            # 🛑 CRITICAL PERMANENT CLEANUP PROTOCOL
            # This block is guaranteed to run even if you hit Ctrl+C right during a sweep
            if proc:
                try:
                    proc.terminate()
                    proc.wait(timeout=2)
